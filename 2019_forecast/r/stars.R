# R script required to implement Sequential T-test Analysis of Regime Shifts Algorithm (Rodionov 2004, 2006) in script.R.
# Modified from Sergei Rodionov's VBA and Andrew Gardner's Matlab codes to produce an R version of STARS. Go to http://www.climatelogic.com/overview for more information, 
# or see the Rodionov (2004; 2006) references in the paper.
...
stars<- function(y=c(rnorm(50), rnorm(50,2,1)), L=20, p=0.05, h=1,  AR1red="none", prewhitening = F) {
  ...
  # Calculates equivalent sample size as in von Storch and Zwiers (1999, p.115)
  ...
  # Compute the rsi for a given sample index, regime mean, and critical threshold.
  rsi<-function(k){
    if(x[k] > (lvl + thresh)){
      r = cusumUp(k)
