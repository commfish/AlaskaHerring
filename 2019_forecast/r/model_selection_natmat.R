...
# Data/Libraries and user inputs ----
...
MODEL_VERSION <- "HER_condEffort_fixedmat"   # HER starting in 1929
...
perms <- gtools::permutations(n = length(combos), # size of source vector 
                              ...
                              r = 1, # number of time-varying parameters
...
## —————————————————————————————————————————————————————————————————————————— ##
## 1     1     4.0     0.3     0     0     2          1980     2018
1     1      3.3     0.8     0     0     3          1980     2018
## ——————————————————————————————————————————————————————————————————————————— ##
...
check_converge <- matrix(ncol = 5, nrow = nrow(perms)) 
...
for(i in 1:nrow(perms)){
  ...
  if (file.exists(std)) {check_converge[i,5] <- 1} else {check_converge[i,5] <- 0 }
}
...
selection_summary %>% 
  ...
  select(Folder_name, Structure, deltaAIC, NLL, AIC, GHL, nopar, max_grad, Survival) %>% 
