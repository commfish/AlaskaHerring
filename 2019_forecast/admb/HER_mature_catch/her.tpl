...
FUNCTION void runForecast()
  ...
  **/
  ...
  fore_vb = fore_matnj * elem_prod(Sij(nyr),data_cm_waa(nyr)(sage,nage)); // vulnerable biomass at age
  ...
  // the effect of the current fishery on next year's spawning stock.
  // incorporated ghl from this year's catch
  // predicted catch-at-age proportions
     dvar_vector pa = elem_prod(fore_matnj,Sij(nyr));
     pa /= sum(pa);
...
FUNCTION void initializeMaturitySchedules() 
  ...
  for(int h = 1; h <= nMatBlocks; h++) {
    ...
    // Reads: if h is not equal to nMatBlocks then nMatBlock(j) else nMatBlockYear(h)-retro_yrs
