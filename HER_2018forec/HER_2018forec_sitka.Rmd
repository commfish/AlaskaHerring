...
```{r posterior_ssb, include = FALSE}

# I don't understand why the credibility intervals are *so* tight on SSB. There is definitely something going wrong here.

ssb <- D$post.samp.ssb
colnames(ssb) <- paste(D$year)
df <- gather(ssb, Year, ssb) %>% 
  group_by(Year) %>% 
...
ggplot(df) +
  ...
  geom_ribbon(aes(x = year, ymin = q025, ymax = q975),
              alpha = 0.2, colour = "grey", fill = "grey") +
  labs(x = "\nYear",
...
```{r HER_weightatage, include = FALSE}



# Two ways of plotting trends in weight-at-age
...
```{r HER_eggdep, include=FALSE}

# Two ways of plotting egg deposition (observed and model predicted, residuals)
...
df %>% 
  ...
  mutate(pred = D[["pred_egg_dep"]],
         # Pearson's residuals
         resid = (obs - pred)/ sqrt(var(pred))) -> df

# 1) observed and fitted values
ggplot(data = df) +
  geom_line(aes(x = Year, y = pred, linetype = "Predicted"), colour = "grey", size = 1) +
  geom_point(aes(x = Year, y = obs, shape = "Observed")) +
  guides(shape = guide_legend(title = NULL, keywidth = 0.1, keyheight = 0.1, default.unit = "inch"), 
         linetype = guide_legend(title = NULL, keywidth = 0.1, keyheight = 0.1, default.unit = "inch")) +
  labs(x = NULL, y = "Egg spawned (trillions)\n") +
  scale_x_continuous(breaks = axis$breaks, labels = axis$labels) +
  theme(legend.position = c(0.1, 0.7),
        legend.direction = "horizontal") -> obsfit

# 2) residuals
ggplot(data = df,aes(x = Year, y = resid)) + 
  geom_hline(yintercept = 0, colour = "grey", size = 1) +
  geom_point() +
  geom_segment(aes(x = Year, xend = Year, y = 0, yend = resid), 
               size = 0.2) +
  labs(x = "\nYear", y = "Pearson residuals\n") +
...
```{r HER_catch, include=FALSE}

# If conditioning model on fishing effort, all elements of D$ft will equal 0


if(sum(as.data.frame(D[["ft"]])) == 0) {
  # if TRUE, model was conditioned on catch, so just plot observed catch
  df <- as.data.frame(D[["data_catch"]])[1:2] 
  colnames(df) <- c("Year", "obs")
  
  ggplot(data = df, aes(x = Year, y = obs)) +
    geom_line() +
    geom_point(aes(x = Year, y = obs)) +
    labs(x = NULL, y = "Catch (tons)\n") +
    scale_x_continuous(breaks = axis$breaks, labels = axis$labels) + 
    scale_y_continuous(labels = scales::comma) -> conditioned_oncatch_plot
  
  ggsave("figs/HER/conditioned_oncatch_plot.png", 
         plot = conditioned_oncatch_plot, 
         dpi = 300, height = 3, width = 6, units = "in")
  
} else {
  
  # if FALSE, model was conditioned on fishing effort. if so plot observed vs
  # fitted catch, residuals, and Ft (fishing effort) time series
  
  df %>% 
    filter(Year >= D[["mod_syr"]]) %>% 
    mutate(pred = D[["pred_catch"]],
           effort = D[["ft"]], 
           # Pearson's residuals
           resid = (obs - pred)/ sqrt(var(pred))) -> df
  ...
  # 1) observed and fitted values
  ggplot(data = df) +
    geom_line(aes(x = Year, y = pred), colour = "grey", size = 1) +
    geom_point(aes(x = Year, y = obs)) +
    labs(x = NULL, y = "Catch\n") +
    scale_x_continuous(breaks = axis$breaks, labels = axis$labels) + 
    scale_y_continuous(labels = scales::comma) -> obsfit
  ...
  # 3) fishing effort
  ggplot(data = df, aes(x = Year, y = effort)) +
    geom_point() +
    labs(x = NULL, y = "Fishing effort\n") +
    scale_x_continuous(breaks = axis$breaks, labels = axis$labels) -> ft 
  
  cowplot::plot_grid(obsfit, resids, ft, align = "hv", nrow = 3) -> conditioned_oneffort_plot
  
  ggsave("figs/HER/conditioned_oneffort_plot.png", 
         plot = conditioned_oneffort_plot, 
...
# Compare quantities of interest between HER and LS

# 1. Time series of mature biomass (pre-fishery) and spawning biomass (post-fishery)
df <- data.frame(Year = D[["year"]], 
                 HER_matB = D[["mat_B"]] / 0.90718,
                 HER_spB = D[["sp_B"]] / 0.90718,
                 LS_matB = LS_byyear$tot_mat_B_tons,
                 LS_spB = LS_byyear$tot_sp_B_tons) %>% 
  gather("Biomass", "tons", -Year)

ggplot(df, aes(x = Year, y = tons, colour = Biomass)) +
  geom_point() +
  geom_line()
