...
# Model convergence and forecast values
...
`r prettyNum(P$nlogl, trim=TRUE, big.mark=",")`

Max gradient (values < 0.001 generally indicate convergence) 
`r prettyNum(P$maxgrad, trim=TRUE, big.mark=",")`

Forecast mature biomass 
`r prettyNum(D$fore_matb, trim=TRUE, big.mark=",")`

Guideline harvest level
`r prettyNum(D$ghl, trim=TRUE, big.mark=",")`

## Mature numbers-at-age
```{r read_matnij, echo=FALSE, message=FALSE, warning=FALSE}
# Mature numbers at age
...
```{r read_catchnij, echo=FALSE, message=FALSE, warning=FALSE}
df <- data.frame(D[["years"]], 
                 D[["Cij"]])
colnames(df) <- c("Year",paste(D[['sage']]:D[['nage']]))
kable(df)
