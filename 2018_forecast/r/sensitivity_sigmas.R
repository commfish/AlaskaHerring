...
# sigmaR and sigmaM (the parameters controling the variability of recruitment
# and time-varying natural mortality) with the 2017 best parameterization of LS.
# For both sigmaR and sigmaM compare low, medium, and high values (9 trials).
# Only create comparison figures for model runs that converged.
...
run_dir <- file.path(root_dir, paste0(YEAR, "_forecast/results/sensitivity_sigmas"))
...
sigmaM_vec <- c(0.001, 0.005, 0.010, 0.025, 0.050, 0.075, 0.100, 0.500)
...
setwd(root_dir)
run_dir <- file.path(root_dir, paste0(YEAR, "_forecast/results/sensitivity_rdevs_sigmaM"))
dir.create(run_dir, showWarnings = FALSE)
...
for(i in 1:nrow(sigma_rdevs_df)){
  for(j in 1:length(sigmaM_vec)){
    ...
    sensdir <- file.path(run_dir, paste0("rdevs_", sigma_rdevs_df$combos[i], 
...
# Remove some of the sigmaM values so the results are a little easier to digest sigmaM=0.2 and 0.4 for remaining visualizations so it's easier to read
...
R + geom_line(aes(y = age3), size = 0.8) + labs(x = "\nYear", y = "Ricker age-3 abundance (millions)\n", colour = "sigma_rdevs")
